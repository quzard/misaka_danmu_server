...
async def webhook_search_and_dispatch_task(
    ...
    try:
        ...
        if fallback_enabled:
            # 顺延机制启用：依次尝试候选源，直到有一个导入成功
            # 评估前3个最佳匹配项，设置最低相似度阈值
            max_candidates = min(3, len(valid_candidates))
            ...
            # 获取符合阈值的候选项
            filtered_candidates = [item for item in candidates_with_similarity if item['similarity'] >= min_similarity_threshold]

            if not filtered_candidates:
                raise ValueError(f"未找到 '{animeTitle}' 的足够相似的匹配项（最低阈值: {min_similarity_threshold}%）。")

            logger.info(f"🔄 Webhook 顺延机制: 已启用，共有 {len(filtered_candidates)} 个候选源待尝试")
            last_error = None
            for attempt, item in enumerate(filtered_candidates, 1):
                candidate = item['candidate']
                logger.info(f"→ [{attempt}/{len(filtered_candidates)}] 尝试候选源: '{candidate.title}' (Provider: {candidate.provider}, 相似度: {item['similarity']}%)")
                await progress_callback(50 + attempt * 10, f"尝试源 {candidate.provider} ({attempt}/{len(filtered_candidates)})")

                current_time = get_now().strftime("%H:%M:%S")
                if mediaType == "tv_series":
                    if currentEpisodeIndex is not None:
                        task_title = f"Webhook（{webhookSource}）自动导入：{candidate.title} - S{season:02d}E{currentEpisodeIndex:02d} ({candidate.provider}) [{current_time}]"
                    else:
                        task_title = f"Webhook（{webhookSource}）自动导入：{candidate.title} - S{season:02d} 整季 ({candidate.provider}) [{current_time}]"
                else:
                    task_title = f"Webhook（{webhookSource}）自动导入：{candidate.title} ({candidate.provider}) [{current_time}]"

                unique_key_parts = ["import", candidate.provider, candidate.mediaId, f"s{season}"]
                if currentEpisodeIndex is not None:
                ...
                try:
                    await generic_import_task(
                        ...
                        season=candidate.season, currentEpisodeIndex=currentEpisodeIndex, imageUrl=candidate.imageUrl,
                        config_manager=config_manager, metadata_manager=metadata_manager,
                    ...
                    if attempt < len(filtered_candidates):
                        logger.info(f"→ Webhook 任务: 继续尝试下一个候选源 ({attempt + 1}/{len(filtered_candidates)})...")
                        last_error = import_error
                        continue
                    else:
                        logger.error(f"✗ Webhook 任务: 所有 {len(filtered_candidates)} 个候选源都导入失败")
                        last_error = import_error
                        break
            ...
            if last_error:
                ...
                if currentEpisodeIndex is not None:
                    ...
                    raise ValueError(f"所有候选源都无法提供第 {season} 季")
        else:
            # 顺延机制关闭，使用多源同时导入逻辑
            ...
            if len(unique_top_matches) > 1:
                ...
                await progress_callback(50, f"准备创建 {total} 个导入任务")
                for idx, (match_item, score) in enumerate(unique_top_matches, start=1):
                    ...
                    # 修正：优先使用搜索结果的年份
                    final_year = match_item.year if match_item.year is not None else year
                    task_coro = lambda session, cb, mi=match_item, fy=final_year: generic_import_task(
                ...
                await progress_callback(90, f"已创建 {len(unique_top_matches)} 个导入任务")
